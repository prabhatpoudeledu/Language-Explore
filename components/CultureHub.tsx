
import React, { useState } from 'react';
import { GeoSection } from './GeoSection';
import { SongSection } from './SongSection';
import { PuzzleSection } from './PuzzleSection';
import { LanguageCode, UserProfile, LANGUAGES } from '../types';

interface Props {
    language: LanguageCode;
    userProfile: UserProfile;
    showTranslation: boolean;
    addXp: (amount: number) => void;
}

type TabId = 'places' | 'songs' | 'puzzles';

export const CultureHub: React.FC<Props> = ({ language, userProfile, showTranslation, addXp }) => {
    const [activeTab, setActiveTab] = useState<TabId>('places');
    const langConfig = LANGUAGES.find(l => l.code === language)!;

    const tabs: { id: TabId; label: string; icon: string; color: string }[] = [
        { id: 'places', label: 'Places', icon: '🏛️', color: 'orange' },
        { id: 'songs', label: 'Songs', icon: '🎵', color: 'pink' },
        { id: 'puzzles', label: 'Puzzles', icon: '🧩', color: 'purple' }
    ];

    return (
        <div className="flex flex-col animate-fadeIn max-w-7xl mx-auto">
            <div className="text-center mb-16">
                <h2 className="text-6xl font-bold text-gray-800 mb-4">{langConfig.menu.discovery} 🎭</h2>
                <p className="text-2xl text-gray-400 font-medium italic">Explore the sights, sounds, and games of {langConfig.country}!</p>
            </div>

            <div className="flex justify-center gap-6 mb-16 bg-white p-4 rounded-[40px] shadow-xl border-2 border-slate-50 self-center">
                {tabs.map(tab => (
                    <button
                        key={tab.id}
                        onClick={() => setActiveTab(tab.id)}
                        className={`px-10 py-5 rounded-[30px] font-bold text-xl flex items-center gap-4 transition-all transform active:scale-95
                            ${activeTab === tab.id 
                                ? `bg-${tab.color}-500 text-white shadow-2xl scale-110` 
                                : `bg-slate-50 text-gray-400 hover:bg-slate-100`
                            }
                        `}
                    >
                        <span className="text-2xl">{tab.icon}</span>
                        {tab.label}
                    </button>
                ))}
            </div>

            <div className="min-h-[600px] animate-fadeIn">
                {activeTab === 'places' && (
                    <GeoSection language={language} userProfile={userProfile} showTranslation={showTranslation} addXp={addXp} />
                )}
                {activeTab === 'songs' && (
                    <SongSection language={language} userProfile={userProfile} showTranslation={showTranslation} addXp={addXp} />
                )}
                {activeTab === 'puzzles' && (
                    <PuzzleSection language={language} userProfile={userProfile} showTranslation={showTranslation} addXp={addXp} />
                )}
            </div>
        </div>
    );
};
