...
import { fetchAlphabet, fetchNumbers, speakText, stopAllAudio, triggerHaptic, unlockAudio, resolveVoiceId, generatePronunciationHints, generateTracingSteps } from '../services/geminiService';
...
export const TracingPracticeSection: React.FC<Props> = ({ language, userProfile, showTranslation, addXp }) => {
  ...
  const buildAiKey = (type: string, target: string) => {
    ...
    return `ai_trace_${language}_${aiOutputLang}_${type}_${activeCategory}_${safeTarget}`;
  };
  ...
  useEffect(() => {
    ...
    setAiHint(null);
    setAiSteps([]);
  }, [activeCategory, selectedLetter?.char, selectedCombo, selectedNumber?.value, aiOutputLang]);
    ...
    const load = async () => {
      ...
      try {
        ...
        const storedKeys = new Set<string>([
          ...Object.keys(stored.letters).map(char => `letter:${char}`),
          ...Object.keys(stored.numbers).map(value => `number:${value}`)
        ]);
        setCompletedTraces(storedKeys);
  ...
  const checkTrace = () => {
    ...
    if (matchRatio >= 0.6) {
      ...
      if (key) {
        ...
        if (activeCategory === 'number' && selectedNumber) {
          markNumberComplete(selectedNumber.value);
        }
        if (activeCategory !== 'number' && selectedLetter) {
  ...
  const getTraceText = () => {
    if (activeCategory === 'number') return selectedNumber?.numeral || '';
    return selectedCombo || selectedLetter?.char || '';
  };

  const handleAiHint = async () => {
    const target = activeCategory === 'number' ? selectedNumber?.word : selectedLetter?.char;
    if (!target) return;
    setActiveAiPanel('hint');
    setIsAiLoading(true);
    setAiError('');
    try {
      const cacheKey = buildAiKey('hint', target);
      const cached = readAiCache<{ romanized: string; syllables: string[]; hint: string }>(cacheKey);
      if (cached) {
        setAiHint(cached);
        return;
      }
      const data = await generatePronunciationHints(target, language, aiOutputLang);
      setAiHint(data);
    ...
    <div ref={sectionTopRef} className="max-w-6xl mx-auto animate-fadeIn pb-16 px-2 md:px-4">
      <div className="text-center mb-6 md:mb-8">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
          <div className="px-4 py-1.5 rounded-full bg-amber-50 text-amber-600 text-xs font-black shadow-sm">{showTranslation ? 'Level' : 'स्तर'} {traceLevel}</div>
          <div className="px-4 py-1.5 rounded-full bg-emerald-50 text-emerald-600 text-xs font-black shadow-sm">{showTranslation ? 'Letters' : 'अक्षर'} {traceLetterCount}</div>
